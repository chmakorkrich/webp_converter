...
read -rp "Enter the path to the image folder (e.g., /home/user/obsidina/vault/images-screenshot): " IMG_DIR
if [[ -z "$IMG_DIR" || ! -d "$IMG_DIR" ]]; then
  echo "Error: Invalid image directory. Exiting."
  exit 1
fi

read -rp "Enter the path to your Obsidian Notes (e.g., /home/user/obsidian/vault/): " NOTES_DIR
if [[ -z "$NOTES_DIR" || ! -d "$NOTES_DIR" ]]; then
  echo "Error: Invalid notes directory. Exiting."
  exit 1
fi

# ----------- Checking if 'cwebp' is installed in Arch or Ubuntu if not it will install. ----------
...
# ---------- Convert .png to .webp and delete original png -----------
...
echo "Optimization is completed!"
